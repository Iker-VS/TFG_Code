...
pub async fn get_image_by_name_handler(path: web::Path<String>) -> impl Responder {
    ...
    if !file_path.exists() {
        write_log(&format!("GET /image/{{filename}} - Imagen no encontrada: {}", filename)).ok();
        return HttpResponse::NotFound().body("Imagen no encontrada");
    }
    let file_data = match fs::read(&file_path) {
        Ok(data) => data,
        Err(_) => {
            write_log(&format!("GET /image/{{filename}} - Error al leer la imagen: {}", filename)).ok();
            return HttpResponse::InternalServerError().body("Error al leer la imagen");
        },
    };
    write_log(&format!("GET /image/{{filename}} - Imagen servida correctamente: {} ({} bytes)", filename, file_data.len())).ok();
    HttpResponse::Ok()
...
pub async fn post_image_handler(
    ...
    if let Err(_) = fs::write(&file_path, &file_data) {
        write_log(&format!("POST /image - Error guardando archivo: {}", file_name)).ok();
